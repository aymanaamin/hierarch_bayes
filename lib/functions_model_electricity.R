...
#' @export
RunBSR_electric <- function(object, h, fmethod = "arima", 
  ...
  fit_adj <- Arima(window(tsl$`Goods Total Lvl 1/02`,end = 2002), order = c(0, 1, 0))
  forecasts.list$`Goods Total Lvl 1/02` <- t(matrix(replicate(pars$n, simulate(fit_adj, future=TRUE, nsim=pars$h)),ncol=1000))
