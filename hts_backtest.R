...
agggts_red <- aggts(tradegts, levels = 14)
colnames(agggts_red) <- substr(colnames(agggts_red), 25,40)
tradegts_reduced <- gts(y = agggts_red,
                        gnames = c("Regions Total", "Countries Total" ,"Goods Total Lvl 1","Goods Total Lvl 2",
                                   "Goods Lvl 1 per Region", "Goods Lvl 2 per Region","Goods Lvl 1 per Country"),
                        characters = list(c(2,2), c(2,1)))

rm(dat_exp,dat_exp_reg,dat_exp_cat,total_exp,
   total_exp_reg,total_exp_cat,agggts_red,total)
...
agg_gts_reduced <- as.list(aggts(tradegts_reduced))
...
all.equal(agg_gts$Total,agg_gts_reduced$Total)
all.equal(agg_hts_reg$AOAU,agg_gts$`Countries Total/AOAU`)
all.equal(agg_hts_cat$`061`,agg_gts_reduced$`Goods Total Lvl 2/061`)
all.equal(agg_hts_reg$EUDE,agg_gts_reduced$`Countries Total/EUDE`)
all.equal(agg_hts_cat$`061101`,agg_gts$`Goods Total Lvl 4/061101`)

rm(agg_gts,agg_hts_reg,agg_hts_cat,agg_gts_reduced)
...
results_wls <- lapply(fmethods, function(fx){
  ...
  lapply(fdate, function(dx){
    ...
    # Run forecasting and aggregation methods
    fcast <- forecast(window(tradegts_reduced, end = dx-1/12),
                      h = tail(horizons,1)*12, 
                      parallel = T,
                      num.cores = ncores,
                      method = "comb",
    ...
    lapply(horizons, function(hx){
      ...
      # Analyze forecast accuracy at different horizons and return summary
      test_wndw <- window(tradegts_reduced, start = dx+hx-1, end = dx+hx-1/12)
      fcast_wndw = window(fcast, start = dx+hx-1, end = dx+hx-1/12)
      accuracy.gts(fcast_wndw, test_wndw)
